/*
 * malloclab - Implemented with an explicit free list allocator to manage
 * allocation of and freeing of memory.
 *
 * Block structures:
 * An explicit list uses the payload to embed pointers to the previous and next
 * free blocks within a free block. The free and allocated block organizations
 * are shown below:
 *
...
#define MINBLOCKSIZE                                                           \
  16 /* Minmum size for a free block, includes 4 bytes for header/footer       \
        and space within the payload for two pointers to the prev and next     \
        free blocks */

// MACROS
/* NOTE: Most of these macros came from the text book on Page 857 (Fig. 9.43).
 * We added the NEXT_FREE and PREV_FREE macros to traverse the free list */
 ...
 *  ____________ _____________ |  PROLOGUE  |                8+ bytes or 2 ptrs
 * |   EPILOGUE  |
 * |------------|------------|-----------|------------|------------|-------------|
...
/*
 * mm_malloc - Allocates a block of memory of memory of the given size aligned
 * to 8-byte boundaries.
 *
 * A block is allocated according to this strategy:
 * (1) If a free block of the given size is found, then allocate that free block
 * and return a pointer to the payload of that block. (2) Otherwise a free block
 * could not be found, so an extension of the heap is necessary. Simply extend
 * the heap and place the allocated block in the new free block.
 */
void *mm_malloc(size_t size) {
  ...
  /* The size of the new block is equal to the size of the header and footer,
   * plus the size of the payload. Or MINBLOCKSIZE if the requested size is
   * smaller.
   */
...
void *mm_realloc(void *ptr, size_t size) {
  ...
  /* Otherwise, we assume ptr is not NULL and was returned by an earlier malloc
   * or realloc call. Get the size of the current payload */
  ...
  // Case 3: Requested size is greater than the current payload size
    ...
    // otherwise allocate a new block of the requested size and release the
    // current block
    bp = mm_malloc(asize);
...
/*
 * find_fit - Attempts to find a free block of at least the given size in the
 * free list.
 *
 * This function implements a first-fit search strategy for an explicit free
 * list, which is simply a doubly linked list of free blocks.
 */
static void *find_fit(size_t size) {
  // First-fit search
  void *bp;
...
/*
 * remove_freeblock - Removes the given free block pointed to by bp from the
 * free list.
 *
 * The explicit free list is simply a doubly linked list. This function performs
 * a removal of the block from the doubly linked free list.
 */
static void remove_freeblock(void *bp) {
  if (bp) {
    if (PREV_FREE(bp))
...
/*
 * coalesce - Coalesces the memory surrounding block bp using the Boundary Tag
 * strategy proposed in the text (Page 851, Section 9.9.11).
 *
 * Adjancent blocks which are free are merged together and the aggregate free
 * block is added to the free list. Any individual free blocks which were merged
 * are removed from the free list.
 */
static void *coalesce(void *bp) {
...
/*
 * place - Places a block of the given size in the free block pointed to by the
 * given pointer bp.
 *
 * This placement is done using a split strategy. If the difference between the
 * size of block being allocated (asize) and the total size of the free block
 * (fsize) is greater than or equal to the mimimum block size, then the block is
 * split into two parts. The first block is the allocated block of size asize,
 * and the second block is the remaining free block with a size corresponding to
 * the difference between the two block sizes.
 */
static void place(void *bp, size_t asize) {
...
//       printf("Consistency error: block %p in free list but marked
//       allocated!", next); return 1;
//     }
//   }
...
//     if(next < mem_heap_lo() || next > mem_heap_hi()) {
//       printf("Consistency error: block %p outside designated heap space",
//       next); return 1;
//     }
//   }
