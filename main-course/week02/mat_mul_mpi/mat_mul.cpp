...
void mat_mul(float *_A, float *_B, float *_C, int _M, int _N, int _K,
  ...
  // MPI_Status = status;
  // int nrows_per_proc = (M - (mpi_world_size - 1))/mpi_world_size;
  ...
  // CHECK_MPI(MPI_Igather(C, nrows_per_proc * N, MPI_FLOAT, C, nrows_per_proc * N, MPI_FLOAT, main_worker, MPI_COMM_WORLD, &gather_request));
  // MPI_Wait(&gather_request, MPI_SUCCESS);
  CHECK_MPI(MPI_Gather(C, nrows_per_proc * N, MPI_FLOAT, C, nrows_per_proc * N, MPI_FLOAT, main_worker, MPI_COMM_WORLD));
  // Free unuse memory to avoid memory leakage
  // free(local_A);
  // if (mpi_rank > 0) {
  //   free(B);
  // }
