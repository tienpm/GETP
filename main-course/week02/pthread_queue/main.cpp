...
MyQueue::MyQueue(int capacity) : capacity_(capacity), size_(0), head_(0) {
  ...
  pthread_mutex_init(&mutex_, nullptr);       // Initialize mutex with Macro
  pthread_cond_init(&not_empty_, nullptr);    // Initialize condition variable with initialize function
...
void MyQueue::Enqueue(float val) {
  ...
  buf_[(head_ + size_) % capacity_] = val;       // circular queue top pointer
  size_ += 1;
  pthread_mutex_unlock(&mutex_);       // Wake up all blocked thread by mutexed thread
  pthread_cond_broadcast(&not_empty_); // Wake up all blocked thread by condition not_empty_
}
...
float MyQueue::Dequeue() {
  ...
  float ans = buf_[head_ % capacity_]; 
  head_ = (head_ + 1) % capacity_;
  size_ -= 1;
  pthread_mutex_unlock(&mutex_);      // Wake up all block thread by mutexed thread 
  pthread_cond_broadcast(&not_full_); // Wake up all blocked thread by condition not_full_
  return ans;
}
