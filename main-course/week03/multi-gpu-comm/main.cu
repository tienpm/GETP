...
void CheckBuffers() {
  ...
  for (int i = 0; i < NUM_GPU; ++i) {
    CHECK_CUDA(cudaSetDevice(i));
    CHECK_CUDA(
        cudaMemcpy(buf, recvbuf[i], nbytes * NUM_GPU, cudaMemcpyDeviceToHost));
...
void AllGatherWithNCCL() {
  ...
  for (int iter = 0; iter < NITER; ++iter) {
    ...
    for (int i = 0; i < NUM_GPU; ++i) {
       ...
       */
      CHECK_NCCL(ncclAllGather(sendbuf[i], recvbuf[i], nbytes / sizeof(int),
                               ncclInt, comms[i], streams[i]));
    }
...
void AllGatherWithMemcpy() {
  ...
  for (int iter = 0; iter < NITER; ++iter) {
    ...
    for (int i = 0; i < NUM_GPU; ++i) {
       ...
       */
      for (int j = 0; j < NUM_GPU; j++) {
        int offset = nbytes / sizeof(int);
        CHECK_CUDA(cudaMemcpy(recvbuf[j] + i * offset, sendbuf[i], nbytes,
                              cudaMemcpyDefault));
      }
    }
    double et = SyncAllGPUsAndGetTime();
