...
def cov_images_optim(A, debug=False):
    ...
    A_com = np.reshape(A, (A.shape[0], -1), order='F')      # shape = (100, 900)
    A_mean = np.mean(A_com, axis=1, keepdims=True)          # shape = (100, 1)
    A_com -= A_mean                                         # Broadcasting    
    covs = np.matmul(A_com, A_com.T)
...
def main():
    ...
    print(f"\nbase mem: {base_mem} / My mem: {mem}")
    print(f"base time: {base_inf_time} / My time: {inf_time}")
